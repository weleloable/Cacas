...
- **Toda tarea que cambie comportamiento sube la versión** (`app.json`,
  ...
  hábito permanente para las tareas que vengan, no sólo para esta. Comprobado
  en vivo que subir sólo el número no basta: Metro cachea el bundle entre
  builds y, si el caché no se invalida, `expoConfig.version` sigue sirviendo
  el número VIEJO aunque `app.json` ya tenga el nuevo (pasó con la 1.0.1, que
  nunca llegó a verse en la web pese a estar bien escrita en el JSON). Por
  eso `build:web` lleva `--clear` de forma permanente
...
- **Toda actualización sube la versión de la app** (ver "Navegación" → nota
  de `expoConfig.version`): hábito permanente para las tareas que vengan, no
  sólo para esta.
